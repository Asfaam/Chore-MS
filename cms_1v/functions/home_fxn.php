...
function display_recently_assigned_chores() {
    ...
    if ($recentAssignments) {
	    ...
	    //$userName = get_full_name($assignment['who_assigned']);
            $userName = get_full_name_from_pid($assignment['img']);
            $dateAssigned = "Date assigned: " . $assignment['date_assign'];
            $choreName = get_chore_name($assignment['cid']);
	    $choreDetails = "<a href='chore_details.php?cid={$assignment['cid']}' class='chore-details-link' title='{$choreName}'>Chore details</a>";
            //$choreDetails = "<a href='chore_details_view.php' class='chore-details-link' title='{$choreName}'>Chore details</a>";
	    $noDateCompleted = "Date completed: N/A  ";
        ...
        echo "<tr><td colspan='4' style='color: #0056b3; text-align: center;'><strong>Nothing to show here</strong></td></tr>";
    }
...
// Function to display chore details
function display_chores_details() {
    // Call the function to retrieve chore details
    $choreDetails = get_all_chore_details(); 
    
    // Check if there are any chore details
    if ($choreDetails) {
        // Start displaying chore details
       echo "<h2 style='color: crimson; text-align:center;'>Chore Details</h2>";
        echo "<table style='color: black; text-align:left;'>";
        echo "<thead><tr><th class='border border-gray-300 p-2' style='width: 16.67%'>Chore Name</th><th class='border border-gray-300 p-2' style='width: 16.67%'>Assigned To</th><th class='border border-gray-300 p-2' style='width: 16.67%'>Assigned By</th><th class='border border-gray-300 p-2' style='width: 16.67%'>Date Assigned</th><th class='border border-gray-300 p-2' style='width: 16.67%'>Date Due</th><th class='border border-gray-300 p-2' style='width: 16.67%'>Date Completed</th></tr></thead>";
        // Loop through each chore detail
        foreach ($choreDetails as $assignment) {
            $choreName = get_chore_name($assignment['cid']);
	    $assignedTo = get_full_name_from_pid($assignment['img']);
	    $assignedBy = get_full_name($assignment['who_assigned']);
	    $dateAssigned = $assignment['date_assign'];
	    ...
	    $noDateCompleted = "Date completed: N/A  ";
            ...
            if ($assignment['sid'] == 3) {
                $dateCompletedTimestamp = strtotime($assignment['last_updated']);
    		$dateCompleted = "Date completed: " . date('Y-m-d', $dateCompletedTimestamp);
                echo "<tr><td class='border border-gray-300 p-2' style='width: 16.67%'>{$choreName}</td><td class='border border-gray-300 p-2' style='width: 16.67%'>{$assignedTo}</td><td class='border border-gray-300 p-2' style='width: 16.67%' >{$assignedBy}</td><td class='border border-gray-300 p-2' style='width: 16.67%'>{$dateAssigned}</td><td class='border border-gray-300 p-2' style='width: 16.67%'>{$dateDue}</td><td class='border border-gray-300 p-2' style='width: 16.67%'>{$dateCompleted}</td></tr>";
            } else {
                echo "<tr><td class='border border-gray-300 p-2' style='width: 16.67%'>{$choreName}</td><td class='border border-gray-300 p-2' style='width: 16.67%'>{$assignedTo}</td><td class='border border-gray-300 p-2' style='width: 16.67%' >{$assignedBy}</td><td class='border border-gray-300 p-2' style='width: 16.67%'>{$dateAssigned}</td><td class='border border-gray-300 p-2' style='width: 16.67%'>{$dateDue}</td><td class='border border-gray-300 p-2' style='width: 16.67%'>{$noDateCompleted}</td></tr>";
            }
        }
    } else {
        // If there are no assignments, display a message within a table row
        echo "<tr><td colspan='6' style='color: #0056b3; text-align: center; font-family: Goudy Old Style; background-color: slategray;'><strong>Nothing to show here!</strong></td></tr>";
    }
    // Close the table body and table
    echo "</tbody></table>";
}
